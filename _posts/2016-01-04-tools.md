...
## Random small tools and libraries

We have some other tools that we use now and then:

#### [sqli.py](https://github.com/niklasb/ctf-tools/sqli.py)

A small library to perform blind SQL injection. If you wonder why we don't just
use [*sqlmap*](http://sqlmap.org/) for that, [just look at the amount of parameters
...
evaluates a boolean SQL expression (possible on the remote end via some

...
A collection of vagrantfiles for quickly creating new virtual machines. Especially the [ctfbox](https://github.com/saelo/boxes/tree/master/ctfbox) is a box that is equipped with many tools you might seek in a VM for a CTF. You can just go `vagrant up` to start the VM and then `vagrant ssh` to ssh into the machine and start the pwnage.
...
Gramarye has a [modified version](https://github.com/Gram21/ctfcode) for Python2 based on [pwntools](https://github.com/Gallopsled/pwntools/).
